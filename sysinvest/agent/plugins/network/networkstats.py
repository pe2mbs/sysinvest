...
from sysinvest.common.interfaces import NetworkData
...
class NetworkInfo( Thread ):
    def __init__( self, interval: t.Optional[t.Union[int,float]] = 1 ):
        super().__init__()
        self.__list = []
    ...
    def dump( self, iface: t.Optional[str] = None ) -> None:
    ...
    def __update( self, iface: str, bytes_recv: int, bytes_sent: int, upload_speed: int, download_speed: int, update_delay: int ) -> None:
        ...
        try:
            ...
            item = NetworkData( Interface = iface,
                                ReceiveBytes = bytes_recv,
                                TransmitBytes = bytes_sent,
                                ReceiveSpeed = upload_speed,
                                TransmitSpeed = download_speed,
                                AverageReceiveSpeed = upload_speed / update_delay,
                                AverageTransmitSpeed = download_speed / update_delay )
            self.__list.append( item )

        except FoundNetworkItem as data:
            # Update the existing item
            item = data.data
            item.ReceiveBytes = bytes_recv
            item.TransmitBytes = bytes_sent
            item.ReceiveSpeed = upload_speed
            item.TransmitSpeed = download_speed
            item.AverageReceiveSpeed = upload_speed / update_delay
            item.AverageTransmitSpeed = download_speed / update_delay

        finally:
            if isinstance( item, NetworkData ):
                # Add it to the history per adaptor
                self.__history.setdefault( iface, [] ).append( item )
    ...
    def getLoadData( self ) -> t.List[ NetworkData ]:
        data = []
        self.__lock.acquire()
        try:
            averages = {}
